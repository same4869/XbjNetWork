...
public class WenbaX509TrustManager implements X509TrustManager {
	private ArrayList<X509TrustManager> mX509TrustManagers = new ArrayList<X509TrustManager>();

	public WenbaX509TrustManager(InputStream keyStore, String keyStorePassword) throws GeneralSecurityException {
	...
	private TrustManagerFactory prepareTrustManagerFactory(InputStream keyStore, String keyStorePassword) throws GeneralSecurityException {
		...
		try {
			...
			Certificate certificate = cf.generateCertificate(keyStore);
			trustStore.setCertificateEntry("ibest0", certificate);
		} catch (IOException e) {
			throw new GeneralSecurityException(e.toString(), e);
		}
